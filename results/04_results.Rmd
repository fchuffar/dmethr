...
for (tcga_project in tcga_projects) {
  ...
  feats[,paste0("momik_lpvals_", tcga_project)] = -log10(feats[,paste0("momik_lpvals_", tcga_project)])
}
...
for (tcga_project in tcga_projects) {
  ...
  for (gse in gses) {
    ...
    gene_set = idx2
    expression_vector = feats[idx,paste0("momik_pvals_", tcga_project)]
...
**(D)** The GSEA enrichment plots show the **depletion** of ***methplusplus* genes** in the `r sum(!is.na(feats[,"momik_betas"]))`-length **all genes** vector of linear regression coefficients. It means that the epression of *methplusplus* genes anticorrelated with the methylation status of the region surrounding their TSS.
**(E)** (resp. **(F)**) The enrichment plot shows the depletion of the  **top `r ntop` upregulated genes** of the model `r models[[1]]` (resp. `r models[[2]]`) in the `r sum(!is.na(feats[,"momik_betas"]))`-length  ***methplusplus* genes** vector of linear regression coefficients.
