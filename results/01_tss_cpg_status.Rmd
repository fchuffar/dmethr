...
mat = get_matbin_from_seq(seq, up_str, dwn_str, stp)
head(mat)
dim(mat)
...
for (j in 2:3) {
  ...
  plot(pca$x[,i], pca$x[,j], xlab=paste0("PC", i, "(", signif(p[i], 3), "%)"), ylab=paste0("PC", j, "(", signif(p[j], 3), "%)"), pch=16, col=col=feats[rownames(pca$x),]$cpg_status%in%"rich"+1)
